/**
 * CreditsEngine.upgradeTier 属性测试
 * 使用基于属性的测试验证升级功能的通用正确性属性
 * 
 * Feature: membership-tier-management
 * Property 1: 等级变更方向验证（升级部分）
 * 
...
describe('CreditsEngine.upgradeTier - Property Tests', () => {
  ...
  describe('Property 1: 等级变更方向验证（升级部分）', () => {
    /**
     * Property 1.1: 升级操作应该只在目标等级高于当前等级时成功
     * 
     * For any user with a current tier and any target tier that is higher,
     * the upgrade operation should succeed.
     * 
     * **Validates: Requirement 1.1**
     */
    it('should succeed when target tier is higher than current tier', async () => {
      // 定义等级列表（按等级从低到高）
      const tierNames = ['free', 'basic', 'pro', 'premium'];
      const tierLevels = [0, 1, 2, 3];
    ...
    /**
     * Property 1.2: 升级操作应该在目标等级等于当前等级时失败
     * 
     * For any user with a current tier, attempting to "upgrade" to the same tier
          ...
          async (tierIndex, initialCredits) => {
            ...
            try {
              ...
              if (error instanceof InvalidTierChangeError) {
                expect(error.message).toContain('Target tier must be higher than current tier');
    ...
    /**
     * Property 1.3: 升级操作应该在目标等级低于当前等级时失败
     * 
     * For any user with a current tier, attempting to "upgrade" to a lower tier
     * should throw InvalidTierChangeError.
     * 
     * **Validates: Requirement 1.7**
     */
    it('should throw InvalidTierChangeError when target tier is lower than current tier', async () => {
      // 定义等级列表
      const tierNames = ['free', 'basic', 'pro', 'premium'];
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (currentTierIndex, targetTierIndex, initialCredits) => {
            ...
            // 创建用户
            const userId = `user-lower-${currentTierIndex}-${targetTierIndex}-${initialCredits}`;
            const user: User = {
              id: userId,
              credits: initialCredits,
              membershipTier: currentTier,
    ...
    /**
     * Property 1.4: 从 null 等级升级到任何定义的等级应该成功
     * 
     * For any user with null membership tier, upgrading to any defined tier
     * should succeed (null is treated as level -1, lower than all defined tiers).
     * 
     * **Validates: Requirement 1.1**
     */
    it('should succeed when upgrading from null tier to any defined tier', async () => {
      // 定义等级列表
      const tierNames = ['free', 'basic', 'pro', 'premium'];
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          // 生成用户初始积分
          fc.integer({ min: 0, max: 10000 }),
          async (targetTierIndex, initialCredits) => {
            const targetTier = tierNames[targetTierIndex];

            // 创建没有会员等级的用户
            const userId = `user-null-${targetTierIndex}-${initialCredits}`;
            const user: User = {
              id: userId,
              credits: initialCredits,
              membershipTier: null,
    ...
    /**
     * Property 1.5: 升级到未定义的等级应该抛出 UndefinedTierError
     * 
     * For any user, attempting to upgrade to a tier that is not defined in the
     * configuration should throw UndefinedTierError.
     * 
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (currentTierIndex, undefinedTier, initialCredits) => {
            ...
            // 创建用户
            const userId = `user-undefined-${currentTierIndex}-${initialCredits}`;
            const user: User = {
              id: userId,
              credits: initialCredits,
              membershipTier: currentTier,
            ...
            try {
              ...
              if (error instanceof UndefinedTierError) {
                expect(error.message).toContain(`Tier '${undefinedTier}' is not defined`);
    ...
    /**
     * Property 1.6: 升级不存在的用户应该抛出 UserNotFoundError
     * 
     * For any non-existent user ID, attempting to upgrade should throw
     * UserNotFoundError.
     * 
    ...
    /**
     * Property 1.7: 升级操作不应该改变用户的其他属性（除了等级、积分和到期时间）
     * 
     * For any successful upgrade operation, only the membershipTier, credits,
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (currentTierIndex, targetTierIndex, initialCredits) => {
            ...
            // membershipExpiresAt 应该保持不变（null）
  ...
  describe('Property 2: 等级字段更新（升级部分）', () => {
    /**
     * Property 2: 等级字段更新
     * 
     * For any successful upgrade operation, the user's membershipTier field
    ...
    /**
     * Property 2.1: 从 null 等级升级应该更新等级字段
     * 
     * For any user with null membership tier, upgrading should update
     * the membershipTier field to the target tier.
     * 
     * **Validates: Requirement 1.2**
     */
    it('should update membershipTier from null to target tier', async () => {
      const tierNames = ['free', 'basic', 'pro', 'premium'];
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (targetTierIndex, initialCredits) => {
            ...
            const userId = `user-null-tier-update-${targetTierIndex}-${initialCredits}`;
            const user: User = {
              id: userId,
              credits: initialCredits,
              membershipTier: null,
            ...
            // Verify the result shows null -> targetTier transition
            expect(result.oldTier).toBe(null);
            expect(result.newTier).toBe(targetTier);

            // Verify the user's membershipTier field is updated from null
            const updatedUser = await adapter.getUserById(userId);
            expect(updatedUser).not.toBeNull();
            expect(updatedUser?.membershipTier).toBe(targetTier);
          }
  ...
  describe('Property 3: 积分上限应用（升级部分）', () => {
    /**
     * Property 3: 积分上限应用
     * 
     * For any successful upgrade operation, the user's credits should be set
     ...
     * 
     * For any successful upgrade operation, regardless of the user's initial
  ...
  describe('Property 4: 交易记录创建（升级部分）', () => {
    /**
     * Property 4: 交易记录创建
     * 
     * For any successful upgrade operation, the system should create a
     * transaction record that documents the credits change and tier change details.
     * 
     * **Validates: Requirement 1.4**
     */
    it('should create transaction record for upgrade operation', async () => {
      const tierNames = ['free', 'basic', 'pro', 'premium'];
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (currentTierIndex, targetTierIndex, initialCredits) => {
            ...
            expect(upgradeTransaction?.userId).toBe(userId);
            expect(upgradeTransaction?.action).toBe('tier-upgrade');
            expect(upgradeTransaction?.amount).toBe(expectedCredits - initialCredits);
            expect(upgradeTransaction?.balanceBefore).toBe(initialCredits);
            expect(upgradeTransaction?.balanceAfter).toBe(expectedCredits);

            // Verify transaction metadata contains tier change details
            expect(upgradeTransaction?.metadata).toBeDefined();
            expect(upgradeTransaction?.metadata?.oldTier).toBe(currentTier);
            expect(upgradeTransaction?.metadata?.newTier).toBe(targetTier);
            expect(upgradeTransaction?.metadata?.oldCredits).toBe(initialCredits);
            expect(upgradeTransaction?.metadata?.newCredits).toBe(expectedCredits);
            expect(upgradeTransaction?.metadata?.creditsDelta).toBe(expectedCredits - initialCredits);
          }
        ),
        { numRuns: 100 }
      );
    ...
    /**
     * Property 4.1: 交易记录应该正确记录负的积分变动
     * 
     * When upgrading to a tier with a lower credits cap than the user's current
     * credits, the transaction should record a negative credits delta.
     * 
     * **Validates: Requirement 1.4**
     */
    it('should record negative credits delta when downgrading credits', async () => {
      const tierNames = ['free', 'basic', 'pro', 'premium'];
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (currentTierIndex, targetTierIndex) => {
            ...
            // Verify transaction record has negative amount
            const transactions = await adapter.getTransactions(userId);
            const upgradeTransaction = transactions.find(
              txn => txn.id === result.transactionId
            );

            expect(upgradeTransaction?.amount).toBe(targetCap - initialCredits);
            expect(upgradeTransaction?.amount).toBeLessThan(0);
          }
        ),
        { numRuns: 100 }
      );
  ...
  describe('Property 6: 未定义等级错误处理（升级部分）', () => {
    /**
     * Property 6: 未定义等级错误处理
     * 
     * For any target tier that is not defined in the configuration,
     * the upgrade operation should throw UndefinedTierError.
     * 
     * **Validates: Requirement 1.6**
     */
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (currentTierIndex, undefinedTier, initialCredits) => {
            ...
            // Attempt to upgrade to undefined tier
            await expect(
              engine.upgradeTier({
                userId,
                targetTier: undefinedTier
    ...
    /**
     * Property 6.1: 从 null 等级升级到未定义等级应该抛出错误
     * 
     * For any user with null membership tier, attempting to upgrade to an
     * undefined tier should throw UndefinedTierError.
     * 
     * **Validates: Requirement 1.6**
     */
    it('should throw UndefinedTierError when upgrading from null to undefined tier', async () => {
      const tierNames = ['free', 'basic', 'pro', 'premium'];
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.integer({ min: 0, max: 10000 }),
          async (undefinedTier, initialCredits) => {
            const userId = `user-null-undef-${undefinedTier}-${initialCredits}`;
            const user: User = {
              id: userId,
              credits: initialCredits,
              membershipTier: null,
            ...
            // Verify user state is unchanged
            const unchangedUser = await adapter.getUserById(userId);
            expect(unchangedUser?.membershipTier).toBe(null);
            expect(unchangedUser?.credits).toBe(initialCredits);
          }
        ),
        { numRuns: 100 }
  ...
  describe('Property 7: 用户不存在错误处理（升级部分）', () => {
    /**
     * Property 7: 用户不存在错误处理
     * 
     * For any non-existent user ID, the upgrade operation should throw
     * UserNotFoundError.
     * 
     * **Validates: Requirement 1.8**
     */
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (nonExistentUserId, targetTierIndex) => {
            ...
            // Attempt to upgrade non-existent user
            await expect(
              engine.upgradeTier({
                userId: nonExistentUserId,
                targetTier
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (nonExistentUserId, anyTier) => {
            ...
            // Attempt to upgrade with any tier (valid or invalid)
            await expect(
              engine.upgradeTier({
                userId: nonExistentUserId,
                targetTier: anyTier
